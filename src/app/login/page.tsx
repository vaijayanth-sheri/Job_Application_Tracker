...
export default function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (error) throw error;
      // The user will be redirected to Google for auth.
    } catch (err: unknown) {
        ...
        <div className="relative w-full max-w-[360px] mx-auto z-10">
          ...
          <div className="relative mt-8 mb-8 flex items-center justify-center">
            <div className="absolute inset-0 flex items-center">
              <div className="w-full border-t border-indigo-200/20"></div>
            </div>
            <div className="relative bg-[#1E1B4B] px-4 text-xs text-indigo-200 uppercase tracking-widest font-semibold">
              Or continue with
            </div>
          </div>
          ...
          <button
            ...
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 px-4 py-3.5 bg-white hover:bg-slate-50 text-slate-700 text-sm font-bold rounded-xl transition-all shadow-sm border border-transparent disabled:opacity-70 disabled:cursor-not-allowed hover:-translate-y-0.5"
          >
